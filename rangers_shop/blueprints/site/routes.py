...
# @site.route('/')
# def shop():
#     return render_template('shop.html') #looking inside our template_folder (site)


#day3

from flask import Blueprint, redirect, flash, render_template, render_template_string, request 
...
def shop():
    ...
    #the same as SELECT * FROM products, list of objects 
...
def update(id):
    ...
    product = Product.query.get(id) #this should only ever bring back 1 iten/object
    ...
    if request.method == 'POST' and updateform.validate_on_submit():
        ...
        flash(f"You have successfully update product {product.name}", category='success')
        return redirect('/')
...
def delete(id):
    ...
    #query our datbase to find that object we want to delete
